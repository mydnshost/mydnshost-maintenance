...
curl --silent --unix-socket /var/run/docker.sock "http:/docker/containers/json" | /scripts/jq -r '.[] | select(.Labels."uk.co.mydnshost.maintenance.db.backup" == "true") | .Id' | while read CID; do
	...
	for DB in ${DBs}; do
	    ...
	    echo "Dump:"
	    /usr/bin/mysqldump -u"${DBUSER}" -h"${DBHOST}" --hex-blob --lock-tables --databases --single-transaction --no-tablespaces --column-statistics=0 "${DB}" > "${LOCATION}/backup-${NOW}.sql" 2> "${LOCATION}/error-${NOW}.txt"
